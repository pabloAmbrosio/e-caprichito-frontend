...
export function AuthStepPassword({ identifier, onBack, onSuccess }: AuthStepPasswordProps) {
  const { login } = useAuth();
      ...
      if (err instanceof AuthApiError) {
        switch (err.code) {
          ...
          case 'GOOGLE_LOGIN_REQUIRED':
            setError('Esta cuenta usa Google. Inicia sesion con Google.');
            break;
          default:
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex items-center gap-2 mb-5">
        ...
        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-surface-overlay border border-stroke text-sm font-bold text-on-surface truncate max-w-[16rem]">
          <svg viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5 text-turquoise shrink-0" aria-hidden="true">
      ...
      <p className="text-center mt-4">
        <button
          type="button"
          className="text-sm text-turquoise hover:text-turquoise-dark font-bold transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-turquoise focus-visible:ring-offset-2 rounded outline-none"
        >
